...
function InvoiceDetailScreen() {
  ...
  const onDeleteProduct = useCallback((prodID: any) => {
    setInvoiceForm((prev: any) => {
      let updatedData = { ...prev };
      const updateProducts = prev.products.filter((prod: any) => prod.id !== prodID);
      const subTotalAmount = sumProductTotal(updateProducts);
      const updateTaxes = getTotalTaxesWithPercent(prev.taxes, subTotalAmount);
      updatedData.products = updateProducts;
      updatedData.taxes = updateTaxes;
  ...
  const handlerInvoiceValue = useCallback((event: any, keyName: any) => {
    const value =
  ...
  const handlerProductValue = useCallback(
    (event: any, keyName: any, productID: any) => {
  ...
  const handlerTaxesValue = useCallback(
    (event: any, keyName: any, taxID: any) => {
      const value = event.target.value;
  ...
  const handlerInvoiceClientValue = useCallback((event: any, keyName: any) => {
    const value =
      typeof event === "string" ? new Date(event) : event?.target?.value;
  ...
  const onDeleteTax = useCallback((taxID: any) => {
    setInvoiceForm((prev: any) => {
      const updateTaxes = prev.taxes.filter((prod: any) => prod.id !== taxID);
  ...
  return (
    <div>
      <div className="p-4">
        <PageTitle 
        title={params.id === "new" ? "New Invoice" : `Invoice Detail ${invoiceForm?.statusName}`}
      ...
      {invoiceForm && (
        ...
        >
          ...
          <div className="py-2 px-4">
            ...
            {invoiceForm?.products?.map((product: any, index: any) => (
              <div
            ...
            >
              ...
              >
                <NumberFormat
                  ...
                  //   </span>
                  // )}
                  renderText={(value: string) => (<span>{value} {invoiceForm?.currencyUnit}</span>)}
                />
              </div>
            </div>
            {/* Subtotal Finished */}
            ...
            {/* Taxes */}
            {invoiceForm?.taxes?.map((tax:any, index: any) => (
              <div
                key={`${index}_${tax.id}`}
                className={
                  isExporting
              ...
              >
                ...
                >
                  <NumberFormat
                    ...
                    //   </span>
                    // )}
                    renderText={(value: string) => (<span className={isExporting ? "text-sm" : "text-base"}> {value} {invoiceForm?.currencyUnit}</span>)}
                  />
                </div>
              </div>
            </div>
