...
export const productSlice = createSlice({
  ...
  initialState,
  reducers: {
    addNewProduct: (state: any, action) => {
    ...
    updateNewProductFormField: (state: any, action) => {
    ...
    onConfirmDeletedProduct: (state) => {
      const newDatas = state.data.filter(
        (product: any) => product.id !== state.deletedID
      );
      state.data = newDatas;
    ...
    onConfirmEditProduct: (state: any, action) => {
      const isFindIndex = state.data.findIndex(
        (product: any) => product.id === state.editedID
      );
      if (isFindIndex !== -1) {
        state.data[isFindIndex] = { ...action.payload };
      }
      state.editedID = null;
      localforage.setItem(PRODUCTS_KEY, [...state.data.map((product:any) => JSON.parse(JSON.stringify(product)))]);
    },
...
export const getAllProductSelector = (state: any) => state.products.data;

export const getProductNewForm = (state: any) => state.products.newForm;

export const getDeletedProductForm = (state: any) => state.products.deletedID;

export const getEditedIdForm = (state: any) => state.products.editedID;

export const getIsOpenProductSelector = (state: any) =>
  state.products.openProductSelector;

export const getSelectedProduct = (state: any) => state.products.selectedProduct;
